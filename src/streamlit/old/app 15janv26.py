...
if slide_id == "contexte":
    ...
    **Décision** : Utilisation de Ewltp comme variable cible
    ...
    with col_a:
        ...
        mean_co2_year = (
            df_all.groupby("year")["Ewltp (g/km)"].mean().reset_index().sort_values("year")
        )
        mean_co2_year = mean_co2_year[mean_co2_year["year"] >= 2017]  # Garder années >= 2017
        ...
        fig_line = px.line(
            ...
            y="Ewltp (g/km)",
            markers=True,
            labels={"year": "Année", "Ewltp (g/km)": "CO₂ moyen (g/km)"},
            title="Évolution des émissions de CO₂"
        )
        fig_line.update_layout(
            plot_bgcolor="#f9f9fa",
            xaxis=dict(dtick=1, range=[2017, mean_co2_year["year"].max()]),  # Axe x à partir de 2017
            height=280,
        ...
        df= load_raw_data()
        co2_by_fuel_bar = df[["Ft", "Ewltp (g/km)"]].copy()
        co2_by_fuel_bar = co2_by_fuel_bar.rename(
            columns={"Ewltp (g/km)": "CO2 (g/km)", "Ft": "Carburant"}
        )

        co2_mean_by_fuel = co2_by_fuel_bar.groupby("Carburant")["CO2 (g/km)"].mean().reset_index()
        ...
        fig_bar = px.bar(
            ...
            title="Émissions CO₂ par type de carburant",
        )
        fig_bar.update_layout(
            yaxis_title="Émissions CO₂ moyennes (g/km)",
    ...
    with col_b:
        ...
        fig = px.histogram(
            ...
            x="Ewltp (g/km)", 
            nbins=40, 
            color_discrete_sequence=["#3597e6"],
            labels={"Ewltp (g/km)":"Émissions CO₂ (g/km)"},
            title="Distribution des émissions CO₂"
        )
        fig.update_layout(
            bargap=0.05, 
            xaxis_title="Émissions CO₂ (g/km)", 
            yaxis_title="Nombre de véhicules",
            plot_bgcolor="#f9f9fa",
            height=665,
            xaxis=dict(range=[0, 500])  # Axe X entre 0 et 500
        )
...
if slide_id == "regression":
    ...
    with col_a:
        ...
        st.dataframe(REG_RESULTS_FULL, use_container_width=True, height=245)
    ...
    with col_b:
        ...
        st.dataframe(REG_RESULTS_REDUCED, use_container_width=True, height=245)
    st.markdown(
        """
    <div class="card" style="margin-bottom: 16px;">
    <div class="metric-big">Meilleur modèle : Random Forest</div>
    ...
    with st.expander("Colonnes structurelles à garder / suppressions", expanded=True):
        cols_keep = [
            "Country",
            "Man",
            "Mk",
        ...
        st.markdown("**Colonnes structurelles à garder**")
        st.dataframe(pd.DataFrame({"Colonne": cols_keep}), use_container_width=True, height=300)

        st.markdown("**Suppression des colonnes suivantes**")
