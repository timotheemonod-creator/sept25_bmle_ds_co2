...
def render_shap_waterfall(model, df_in: pd.DataFrame, class_idx: int) -> None:
    df_plot = df_in.copy()
    if "Ft" in df_plot.columns:
        df_plot["Ft"] = df_plot["Ft"].astype("category")

    dmatrix = xgb.DMatrix(df_plot, enable_categorical=True)

    explainer = shap.TreeExplainer(model.get_booster())
    shap_values = explainer(dmatrix)

    # Multiclass: shap_values.values -> (n_samples, n_features, n_classes)
...
if slide_id == "contexte":
    ...
    st.markdown(
        """
    <div class="muted">
        Le projet s'appuie sur le dataset <b>EEA CO₂ cars</b> (European Environment Agency) couvrant 
        la période 2010-2024, avec un focus sur l'année 2022.
    </div> 
    """,
        unsafe_allow_html=True,
    )
    ...
    with col1:
        st.metric("2010-2024", ">91M")
        st.caption("40 colonnes initiales")
    with col2:
        st.metric("Focus 2022", "9,48M")
        st.caption("Volume exploitable")
    with col3:
        st.metric("Doublons supprimés", "533K")
        st.caption("17 colonnes")
    with col4:
        st.metric("Cible Ewltp priorisée", "1% NA")
        st.caption("Enedc ≈ 82% NA")
    st.markdown("---")
    ...
    st.markdown("""
        ...
        <b>Emissions WLTP en g/km</b><br>
        <ul>
        <li><b>≈ 1% de valeurs manquantes</b> → excellente complétude</li>
        <li>Cycle WLTP (Worldwide Harmonized Light Vehicles Test Procedure)</li>
        <li>Standard européen depuis 2017</li>
        </ul>
        <b>Enedc (ancien cycle NEDC)</b>
        <li><b>≈ 82% de valeurs manquantes</b> → obsolète</li>
        <li>Ancien cycle de test, remplacé par WLTP</li>
        </div>
        """,
        unsafe_allow_html=True,)
    ...
    <li>Taux de NA > 80%</li>
    <li>Variables dérivées de la cible</li>
...
if slide_id == "preprocess":
    ...
    with col_b:    # Pipeline de pré-processing
        st.markdown("### Pipeline de pré-processing")
        st.markdown("""
    **Étapes du pipeline :**
    
    1. Chargement des données
    2. Suppression des doublons
    3. Harmonisation des libellés
    4. Nettoyage des faux NA
    5. Split train/test (80/20)
    6. Imputation (numériques et qualitatives)
    7. Encodage ( )
    8. Standardisation (StandardScaler)
    9.discretisation de la variable cible 
    ...
    with col_a:
        ...
        **Stratégies d'imputation :**
        ...
        - Justification : Conserver la cohérence par groupe
        
        **Indicateurs de manquants :**
        - Création de variables binaires pour certaines variables clés
        - Permet au modèle de distinguer "vraiment manquant" vs "valeur imputée"
    ...
    with col_b:
        ...
        st.markdown("""
        **Nettoyage préalable :**
        - Harmonisation des libellés (constructeurs, modèles)
        - Exemple : "BMW", "B.M.W.", "bmw" → "BMW"
        
        **Imputation :**
        - Mode groupé (ex. Man par Mk ou Cn)
        - Justification : Cohérence sémantique
...
if slide_id == "demo":
    ...
    """,
            ...
            if model is not None and not use_demo and pred_class is not None:
                st.markdown("### Explication locale (SHAP)")
                try:
                    render_shap_waterfall(model, df_in, int(pred_class))
