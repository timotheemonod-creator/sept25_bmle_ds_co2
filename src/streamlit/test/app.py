...
def load_model(uploaded: io.BytesIO | None):
    model_paths = [
        BASE_DIR / "models" / "xgb_pipeline.joblib",
        BASE_DIR / "models" / "xgb_classifier.joblib",
        BASE_DIR / "models" / "xgb_classifier.pkl",
    ]
...
def load_preprocessors():
    for base in MODEL_DIRS:
        ohe_path = base / "ohe_low_card.joblib"
        hash_path = base / "hash_preprocessor.joblib"
        scaler_path = base / "scaler.joblib"
        low_card_path = base / "low_card.joblib"
        high_card_path = base / "high_card.joblib"
        feature_cols_path = base / "feature_columns.joblib"
        ...
        if all(
            ...
            return {
                "ohe": joblib_load(ohe_path),
                "hash_preprocessor": joblib_load(hash_path),
                "scaler": joblib_load(scaler_path),
                "low_card": joblib_load(low_card_path),
                "high_card": joblib_load(high_card_path),
                "feature_columns": joblib_load(feature_cols_path),
                "base": str(base),
            }
    return None
...
def preprocess_input(df: pd.DataFrame) -> pd.DataFrame:
    artifacts = load_preprocessors()
    if artifacts is None:
        raise ValueError(
            "Préprocesseurs introuvables. Placez ohe_low_card.joblib, "
            "hash_preprocessor.joblib, scaler.joblib, low_card.joblib, "
            "high_card.joblib, feature_columns.joblib dans `models/` ou "
            "`notebooks/models/`."
        )

    low_card = artifacts["low_card"]
    high_card = artifacts["high_card"]
    ohe = artifacts["ohe"]
    hash_preprocessor = artifacts["hash_preprocessor"]
    ...
    X_ohe = ohe.transform(X[low_card])
    X_ohe = pd.DataFrame(
        X_ohe, columns=ohe.get_feature_names_out(low_card), index=X.index
    )
    X = pd.concat([X.drop(columns=low_card), X_ohe], axis=1)

    X_hash = hash_preprocessor.transform(X)
    X_hash = pd.DataFrame(
        X_hash,
        columns=hash_preprocessor.get_feature_names_out(),
        index=X.index,
    )
    X = pd.concat([X.drop(columns=high_card), X_hash], axis=1)
...
if slide_id == "demo":
    ...
    with col_left:
        with st.form("demo_form"):
            ...
            width_mm = st.selectbox("W (mm)", list(range(1500, 3000, 10)))
    ...
    with col_right:
        if submitted:
            payload = {
                ...
                "At2 (mm)": at2_mm,
                "Ft": fuel_type,
                "Fm": "P",  # motorisation par défaut
                "ec (cm3)": cyl_cm3,
                "ep (KW)": power_kw,
                "z (Wh/km)": 0.0,
                "IT": it_flag,
                "Erwltp (g/km)": np.nan,
                "Fuel consumption": np.nan,
            ...
            try:
                ...
                needs_preprocess = (
                    ...
                    and any(
                        name.startswith(("he_", "Country_", "Ft_", "Fm_"))
                        or name.endswith("_missing")
                        for name in feature_names
                    )
                )
                if hasattr(model, "named_steps") and not needs_preprocess:
                    pred = model.predict(df_in)
                else:
                    df_pre = preprocess_input(df_in)
                    pred = model.predict(df_pre)
                pred_class = str(pred[0])
            except Exception as e:
                st.error(f"Prédiction modèle échouée: {e}")
                st.stop()

            if pred_class is None:
                st.error("Prédiction modèle échouée")   
                st.stop()
